...
4. Get `PYTHONPATH`
    ...
    ```
5. Run baseline, i.e., PyG extended to support disk-based processing of graph dataset (denoted as PyG+ in the paper). Replace `PYTHONPATH=...` with the outcome of step 4. `-W ignore` option is used to ignore warnings.Replace python3 with `$(which python)`.
    ```console
    sudo PYTHONPATH=/home/user/.local/lib/python3.8/site-packages cgexec -g memory:8gb python3 -W ignore run_baseline.py
    ```
6. Run Ginex. Replace `PYTHONPATH=...` with the outcome of step 4. `-W ignore` option is used to ignore warnings.Replace python3 with `$(which python)`.
    ```console
